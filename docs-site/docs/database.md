...
-- Check if a user has claimed coins today
SELECT coins, last_claimed FROM users WHERE id = 1;

-- Get friend list
SELECT * FROM friends WHERE user_id = 1 AND status = 'accepted';

-- Increment habit progress
UPDATE habits SET progress_count = progress_count + 1, last_updated = NOW()
WHERE id = 5;
